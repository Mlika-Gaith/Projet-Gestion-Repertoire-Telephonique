...
LRESULT CALLBACK RechercheVilleAffichageProcedure (HWND hwnd, UINT msg, WPARAM wp , LPARAM lp){
    ...
    case WM_SIZE:
        ...
        GetWindowRect(recherche_ville_affichage_fenetre, &MainRect);
    ...
    case WM_CLOSE:
        DestroyWindow(recherche_ville_affichage_fenetre);
        break;
    default:
...
void registerRechercheVilleAffichageFenetreClass(HINSTANCE hInstance){
    ...
    affichage.lpszClassName = L"AffichageRechercheVille";
    affichage.lpfnWndProc = RechercheVilleAffichageProcedure;
...
HWND displayRechercheVilleAffichageFenetre (HWND hwnd, repertoire rt , char ville [35]){
    ...
    recherche_ville_affichage_fenetre = CreateWindowW(L"AffichageRechercheVille",L" Contenu du Repertoire Telephonique ", WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL,
    ...
    hListView = CreateWindowEx(0, WC_LISTVIEW, 0, WS_CHILD |WS_VISIBLE | WS_BORDER | LVS_ALIGNLEFT | LVS_REPORT | LVS_EDITLABELS ,0,0,MainRect.right - MainRect.left,
                                     MainRect.bottom - MainRect.top,recherche_ville_affichage_fenetre,NULL, NULL, NULL);
    ...
    while (r2 != NULL){
        ...
        if (strcasecmp(r2->cont.adr.ville,ville) == 0){
    ...
    return recherche_ville_affichage_fenetre;
